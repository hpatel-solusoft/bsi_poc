...
def reject_inference(
    ...
    # and the CS-4 warm CASE_STORE never hold rules_fired at all (see
    # core.persistence_filters.strip_graph_derived_fields) — every route
    # (/intake, /similar_cases, /risk_assessment, /plan) re-reads it live
    # from Neo4j on every request instead, so the Neo4j write above is
    # immediately visible everywhere without a second, easy-to-drift write
...
def revert_rejection(
    ...
    # reject_inference above; Neo4j is already the authoritative write and
    # every route re-reads it live.
    return {
        "result": {
            "reverted": True,
            "case_id": case_id,
