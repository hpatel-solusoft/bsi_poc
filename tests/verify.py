...
def test_normalizers():
    ...
    check(
        ...
        N.employer_key("04-1234567", None, "Acme Corp")
        == N.employer_key("041234567", None, "ACME CORPORATION"),
    ...
    check(
        ...
        N.alias_value("  Johnny B  ") == "Johnny B"
        and N.alias_value("johnny b") != N.alias_value("Johnny B"),
    )
    check(
    ...
    check(
        "boolean parses AppWorks' several spellings",
...
def test_rule_engine():
    ...
    check(
        "a disabled rule is reported skipped, not as a miss",
    ...
    check(
        "scope is passed into every rule",
    ...
    check(
        "registry parameters reach the rule that needs them",
        session.calls[1]["params"]["fasttrack_fraud_threshold"] == 50000.0,
    )
    check(
        "asserted_at is stamped once per wave, not per rule",
...
def test_rules_fired():
    ...
    from reasoning_layer import rule_registry, rules_fired
    ...
    with mock.patch.object(rules_fired, "get_session", fake_session_cm(session)):
        block = rules_fired.build_rules_fired(
            scope,
            [{"rule_id": "Rule_05_Alias_Identity", "writes": 0, "skipped_reason": "disabled_in_registry"}],
        )

    check("the block always has all 14 entries", len(block) == 14, f"got {len(block)}")
    check(
        "every entry carries A.4's four required fields",
        all({"rule_id", "fired", "confidence", "corroborated"} <= set(e) for e in block),
    )
    fired = [e for e in block if e["fired"]]
    check(
        "a rule with graph evidence reports fired=true",
        len(fired) == 1 and fired[0]["rule_id"] == "Rule_01_Shared_Employer",
    )
    check(
        "confidence is the strongest tier found, not the first",
        fired[0]["confidence"] == "High",
        f"got {fired[0]['confidence']}",
    )
    ...
    check(
        "a rule that did not fire reports Unresolved, not a stale confidence",
    ...
    check(
        "a disabled rule carries its skipped_reason into the block",
        skipped["skipped_reason"] == "disabled_in_registry",
    )
    check(
        "rule 14 is present and marked as a non-wave modifier",
...
def test_graph_load():
    ...
    def responder(query, params):
        ...
        if "confirms_relationship_ids" in query:
            return (
                {"confirmed_ref": params["relationship_ref"]}
                if params["relationship_ref"] == "REL_REAL"
                else None
            )
        return None
    ...
    extraction = {
        ...
        "corroborations": [
            {
                "relationship_ref": "REL_REAL",
                "comment_ref": "c1",
                "rationale": "comment says they work together",
            },
    ...
    check(
        "a valid attribution is written",
        len(result["written"]) == 1 and result["written"][0]["allegation_id"] == "AL_OK",
    )
    check(
        "a rejected attribution is suppressed, with an investigator-visible note",
        len(result["suppressed"]) == 1 and "rejected by j.doe" in result["suppressed"][0]["note"],
        "Principle 14: suppress, never silently delete",
    )
    check(
        "a hallucinated id is dropped, not written and not counted as suppressed",
    ...
    check(
        "a corroboration referencing a non-existent relationship is dropped",
        result["corroborations_linked"] == 1,
        "the fake REL_FAKE must not be counted",
    )
...
def test_pipeline():
    ...
    with mock.patch.object(
        pipeline.pipeline_state_repository, "get_run_state", return_value=None
    ), mock.patch.object(pipeline.pipeline_state_repository, "start_run"), mock.patch.object(
        pipeline.pipeline_state_repository, "mark_wave1_complete"
    ), mock.patch.object(
        pipeline.pipeline_state_repository, "mark_extraction_complete"
    ), mock.patch.object(
        pipeline.pipeline_state_repository, "mark_wave2_complete"
    ), mock.patch.object(
        pipeline.pipeline_state_repository, "mark_failed"
    ) as failed, mock.patch.object(
        pipeline.rule_registry, "ensure_registry"
    ), mock.patch.object(
        pipeline.rule_registry, "load_registry", return_value={}
    ), mock.patch.object(
        pipeline.scope_resolver, "resolve_scope", return_value=scope
    ), mock.patch.object(
        pipeline.rule_engine, "run_wave1", return_value=[{"rule_id": "Rule_01_Shared_Employer", "writes": 1}]
    ) as w1, mock.patch.object(
        pipeline.rule_engine,
        "run_wave2",
        return_value=[{"rule_id": "Rule_02_Employer_Fraud_Network", "writes": 1}],
    ) as w2, mock.patch.object(
        pipeline.rule_engine,
        "run_modifier",
        return_value=[{"rule_id": "Rule_14_Confirmation_Elevation", "writes": 0}],
    ) as w14, mock.patch.object(
        pipeline, "_run_extraction_stage", return_value={"attributions_extracted": 1}
    ) as ext, mock.patch.object(
        pipeline.rules_fired,
        "build_rules_fired",
        return_value=[{"rule_id": "Rule_01_Shared_Employer", "fired": True}],
    ):
    ...
    check(
        "Wave 2 runs AFTER extraction (its attribution edges must exist first)",
    ...
    completed = {
        ...
        "wave1_completed_at": "x",
        "wave2_completed_at": "y",
    }
    with mock.patch.object(
        pipeline.pipeline_state_repository, "get_run_state", return_value=completed
    ), mock.patch.object(pipeline.pipeline_state_repository, "start_run") as start2, mock.patch.object(
        pipeline.scope_resolver, "resolve_scope", return_value=scope
    ), mock.patch.object(
        pipeline.rules_fired, "build_rules_fired", return_value=[]
    ), mock.patch.object(
        pipeline.rule_engine, "run_wave1"
    ) as w1b:
        out2 = pipeline.run_pipeline("C1", "S1")["result"]

    check(
        "Principle 10: a completed run is skipped, not re-run",
        out2["pipeline_status"] == "already_completed" and not w1b.called and not start2.called,
    )
    check("a skipped run still returns rules_fired (callers never special-case it)", "rules_fired" in out2)

    # (c) force=True (the ETL path) DOES re-run
    with mock.patch.object(
        pipeline.pipeline_state_repository, "get_run_state", return_value=completed
    ), mock.patch.object(pipeline.pipeline_state_repository, "clear_run") as cleared, mock.patch.object(
        pipeline.pipeline_state_repository, "start_run"
    ), mock.patch.object(
        pipeline.pipeline_state_repository, "mark_wave1_complete"
    ), mock.patch.object(
        pipeline.pipeline_state_repository, "mark_extraction_complete"
        ...
        pipeline.rule_engine, "run_wave1", return_value=[]
    ) as w1c, mock.patch.object(
        pipeline.rule_engine, "run_wave2", return_value=[]
    ), mock.patch.object(
        pipeline.rule_engine, "run_modifier", return_value=[]
    ), mock.patch.object(
        pipeline, "_run_extraction_stage", return_value={}
    ), mock.patch.object(
        pipeline.rules_fired, "build_rules_fired", return_value=[]
    ):
    ...
    check(
        "force=True re-runs after a fresh ingest (stale conclusions over new data)",
    ...
    with mock.patch.object(
        ...
        pipeline.pipeline_state_repository, "mark_extraction_complete"
    ) as mx2, mock.patch.object(
        pipeline.pipeline_state_repository, "mark_failed"
    ) as failed2, mock.patch.object(
        pipeline.rule_registry, "ensure_registry"
    ), mock.patch.object(
        pipeline.rule_registry, "load_registry", return_value={}
    ), mock.patch.object(
        ...
        pipeline, "_run_extraction_stage", side_effect=ValueError("bad LLM JSON")
    ), mock.patch.object(
        pipeline.rule_engine, "run_wave2"
    ) as w2d:
        try:
    ...
    check(
        "Principle 15: a built stage failing marks the run failed and re-raises",
...
def test_ingest_service():
    ...
    with mock.patch.object(ingest_service.graph_sync, "sync_case", side_effect=fake_sync), mock.patch.object(
        ingest_service.pipeline, "run_pipeline", side_effect=fake_pipeline
    ), mock.patch.object(ingest_service, "_subjects_for_case", return_value=["S1"]), mock.patch.object(
        ingest_service.graph_ingest_repository, "mark_started"
    ), mock.patch.object(
        ingest_service.graph_ingest_repository, "mark_loaded"
    ), mock.patch.object(
        ingest_service.graph_ingest_repository, "mark_reasoned"
    ), mock.patch.object(
        ingest_service.graph_ingest_repository, "mark_failed"
    ), mock.patch.object(
        ingest_service.time, "sleep"
    ):
    ...
    check(
        "EVERY case is loaded before ANY case is reasoned",
...
def test_wiring():
    ...
    check(
        "the reasoning pipeline is NOT registered as an LLM-callable tool",
    ...
    check(
        "wave membership is by list, and Rules 7/8 are in Wave 2 (not a numeric range)",
    ...
    check(
        "Rule 8 runs after Rule 7 (it reads Rule 7's output)",
    ...
    check(
        "Rule 13 runs after Rule 7 (it reads Rule 7's output)",
        rule_registry.WAVE_2_RULE_IDS.index("Rule_13_FastTrack_Escalation")
        > rule_registry.WAVE_2_RULE_IDS.index("Rule_07_Prior_Guilty"),
    )
    check(
        "Rule 14 is not a wave member",
    ...
    check(
        "every rule checks for a rejection before re-asserting (Section 5.5)",
        not no_guard,
        f"missing guard: {no_guard}",
    )
