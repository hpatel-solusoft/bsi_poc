...
def _build_case_summary(wf_props: dict) -> str:
    ...
    parts = []
    if allegation_desc:
        parts.append(f"Allegation Details: {allegation_desc}")
    if analyst_note:
        parts.append(f"Analyst Notes: {analyst_note}")
    if reviewer_note:
        parts.append(f"Reviewer Notes: {reviewer_note}")

    if not parts:
        fallback = wf_props.get("WorkfolderDescription") or wf_props.get("Workfolder_CaseDescription") or ""
        if fallback:
            parts.append(f"General Description: {fallback}")
...
def search_similar_cases(case_id: str, fraud_types: list, max_total_results: int = 3, **kwargs) -> dict:
    """Find closed cases with matching fraud types, scored and ranked for the investigator."""
    ...
    for type_id, type_desc in allegation_types:
        ...
        for res in results:
            ...
            type_candidates.append({"wf_id": wid, "wf_props": props, "date_received": date_received})
        ...
        logger.info(
            f"Candidates fetched and sorted for type '{type_desc}' | active_case: {case_id} | candidates_found: {len(type_candidates)}"
        )
    ...
    logger.info(
        f"Similar case search completed | active_case: {case_id} | total_candidates_scored: {len(candidates)}"
    )
    final_matches = candidates[:max_total_results]
    logger.info(
        f"Final similar cases selected | active_case: {case_id} | matches_returned: {len(final_matches)}"
    )
    return {
        "result": {
...
def get_allegation_types(**kwargs) -> dict:
    """Fetch the full AppWorks AllegationType catalogue (tool-callable)."""
