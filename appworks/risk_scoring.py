...
def _fetch_child_rules_breakpoints(
    item_id: str, dimension_key: str, tracker: ProvenanceTracker, child_href: Optional[str] = None
) -> List[Dict]:
    ...
    try:
        ...
        for child in child_items:
            ...
            cond_str = str(
                cp.get("CONDITION") or cp.get("Condition") or cp.get("AgentRulesTable_Condition") or ""
            )
            wt_str = (
                cp.get("WEIGHT")
                or cp.get("Weight")
                or cp.get("POINTS")
                or cp.get("Points")
                or cp.get("AgentRulesTable_Weight")
            )
        ...
        numeric = sorted(
            [t for t in thresholds if "min_value" in t], key=lambda t: t["min_value"], reverse=True
        )
            ...
            for alleg_item in alleg_items:
                ...
                if not type_href:
                    ...
                    if a_self:
                        ...
                        type_href = a_links.get("relationship:Allegations_AllegationsType", {}).get(
                            "href", ""
                        )
...
def _build_case_context(
    ...
    try:
        ...
        for i in fin_items:
            ...
            if f_self:
                tracker.add_source("Financial", extract_id_from_href(f_self))
    ...
    fast_track = bool(
        wf_props.get("WorkfolderFastTrack") or wf_props.get("FAST_TRACK") or wf_props.get("FastTrack")
    )
...
def _evaluate_numeric(rule: RiskRuleDef, context: Dict) -> TriggeredRule:
    ...
    if rule.thresholds:
        numeric_bps = sorted(
            [t for t in rule.thresholds if t.min_value is not None], key=lambda x: x.min_value, reverse=True
        )
    ...
    return TriggeredRule(
        ...
        weight=min(total_weight, rule.max_pts),
            ...
            if m:
                ...
                if actual_val is not None:
                    ...
                    try:
                        ...
                        target_str, actual_str = (
                            target_val_str.strip("'\"").lower(),
                            str(actual_val).strip().lower(),
                        )
                        if operator in ("==", "="):
                            matched = actual_str == target_str
                        elif operator == "!=":
                            matched = actual_str != target_str
...
def get_risk_rules(**kwargs) -> Dict:
    ...
    logger.info("TOOL CALL: get_risk_rules -> Fetching from AppWorks...")
...
def _fetch_risk_rules(tracker: Optional[ProvenanceTracker] = None) -> list[Dict]:
    ...
    try:
        ...
        for idx, item in enumerate(items):
            ...
            rule_name = str(
                props.get("RULE_NAME") or props.get("RuleName") or props.get("Name") or ""
            ).strip()
            rule_desc = str(
                props.get("RULE_DESC") or props.get("DESCRIPTION") or props.get("Description") or ""
            ).strip()
            ...
            raw_targets = (
                props.get("TARGETD_FRAUD_TYPE")
                or props.get("TARGET_FRAUD_TYPES")
            ...
            if item_id or child_href:
                parsed_thresholds = _fetch_child_rules_breakpoints(
                    str(item_id) if item_id else "", dimension_key, tracker, child_href=child_href
                )
...
def calculate_risk_metrics(case_id: str, subject_id: str, fraud_types: List, **kwargs) -> Dict:
    ...
    for raw_rule in active_rules:
        ...
        if rule.target_fraud_types:
            case_types = context.get("fraud_types", [])
            has_match = any(
                ct in target or target in ct for ct in case_types for target in rule.target_fraud_types
            )
            if not has_match:
                continue
