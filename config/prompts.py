...
The tool you call returns a structured investigation plan — steps, evidence

checklist, and escalation criteria. Treat this as raw material, not final output.

After the tool returns, use both the plan and the verified case context above

to produce a fully synthesised, case-specific investigation strategy. 
...
- Produce one clearly labelled section for each distinct category of data returned

  by the tool. Derive each section title from the nature of the data — not from

  internal system names or identifiers.
 
- Do not truncate. Do not summarise. Every item returned by the tool must be fully

  covered in its own right — not compressed, not reduced to a count.
...
- For any escalation condition returned: state it precisely in plain language so

  the investigator knows exactly what would change the course of the investigation.
...
Write in plain English. Every claim must be grounded in the tool result or the

verified case context above. Do not reproduce system identifiers, field names,

or raw data structures anywhere in the analysis.
...
Third — what this means for the investigator holding this specific case right now.

What does the archive tell them that changes how they should approach this
...
an investigator can pull themselves. The interpretation — grounded in this specific

case — is what they cannot.
 
Do not fabricate. If information is absent from the results, state it is not recorded."""
}
