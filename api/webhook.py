...
def _rehydrate_case_store(case_id: str, ai_summary: dict) -> None:
    ...
    for key in ["similar_cases", "risk_rules", "risk_assessment", "investigation_playbook"]:
...
def similar_cases(req: PlaybookRequest):
    ...
    try:
        ...
        similar_tools = [
            tool
            for tool in runner.auto_tools
            if tool["function"]["name"] in {"search_similar_cases"}
        ]
...
def playbook(req: PlaybookRequest):
    """
    ON-DEMAND — Playbook Route (Step 4 in flow).
    ...
    try:
        ...
        playbook_tools = [
            tool
            for tool in runner.on_demand_tools
            if tool["function"]["name"] in {"get_investigation_playbook"}
        ]
        messages, new_provenance, _ = runner.run_scoped(
        ...
        # Parse markdown prose into structured fields (Service-layer logic to fill playbook)
        ...
        # Build structured playbook from parsed prose
        # Start with metadata from tool result if available
        playbook_result = sections.get("investigation_playbook", {})
        
        import re
        id_match = re.search(r"Case\s*(?:ID|#)?\s*[:\s]*(\d+)", assistant_text, re.I)
        cid = id_match.group(1) if id_match else req.case_id
        playbook_id = playbook_result.get("playbook_id") or f"PLAN-{cid}-{datetime.now().strftime('%Y%m%d')}"

        investigation_playbook = {
            "playbook_id": playbook_id,
            "fraud_types": playbook_result.get("fraud_types") or case_data.get("complaint_intelligence", {}).get("fraud_types", []),
            "risk_tier": playbook_result.get("risk_tier") or case_data.get("risk_assessment", {}).get("risk_tier", ""),
        ...
        investigation_data = {**case_data}
        investigation_data.pop("provenance_trail", None)
        investigation_data.pop("investigation_playbook", None) # Dedup
