...
Internal structure (post-refactor, behavior unchanged): each of
run_generate_report / run_generate_report_pdf is now a thin
orchestrator over its OWN pair of private helpers
(_generate_report_try_cache/_generate_report_generate_fresh and
_generate_report_pdf_try_cache/_generate_report_pdf_generate_fresh,
respectively) — deliberately NOT shared with the other route, for the
same reason the two top-level functions themselves aren't merged (see
above). The one exception is _log_call: a trivial audit-log kwargs
wrapper with no pipeline logic in it, shared by both routes the same
way both already called the same core.agent_audit_repository.log_agent_call
before this refactor — sharing it does not create the kind of coupling
the "deliberate duplicate" note above is guarding against.
"""
...
def _log_call(case_id: str, username: str, status: str, latency_ms: int, endpoint: str) -> None:
    """Single call site for the report-route audit-log write. Both
    routes share it (see module docstring) since it carries no pipeline
    logic — only status/latency/endpoint ever vary."""
    log_agent_call(
        case_id=case_id,
...
def _generate_report_try_cache(
    ...
    plan_override,
    start: float,
) -> tuple[Optional[Dict[str, Any]], StalenessCheck]:
    """Report cache: if a report has already been generated and
    persisted for this case_id AND neither staleness trigger fired,
    answer from the latest report_artifacts row WITHOUT calling the LLM
    again.

    /generate_report's "graph changed" signal is the OR of TWO
    independent detectors, deliberately kept BOTH rather than one
    replacing the other:
      1. The existing, finer-grained content diff: is the live Related
         Network (passed in as `related`) byte-identical to what was
         cached? This predates AI-32 and stays authoritative for the
         actual cache-serve decision — it catches the exact set of cases
         that matter for THIS route (a connection's status/membership
         actually differs) and is immune to false positives from a
         reject immediately followed by a revert (net content unchanged,
         but AI-31's timestamp would still have moved).
      2. AI-32's new (:Case).last_inference_change_at vs this report's
         own generated_at — the same coarse signal every other
         cached-narrative route now uses. Folded in with OR, never
         replacing #1: a defense-in-depth signal, not a downgrade of the
         existing precision. Practically this only ever fires ahead of
         #1 in the reject-then-revert case above, where reporting
         stale=True even though the content is provably unchanged is
         the honest answer — an investigator DID touch the graph —
         while this route still correctly serves the (genuinely
         still-accurate) cached content either way.
    Any actual difference means the cached narrative prose (Reviewed
    and Excluded Connections, Network Connections) can no longer be
    trusted — that text was written by the LLM once, at generation
    time, and there is no safe way to splice a per-connection narrative
    sentence the way the deterministic Decision & Override Log block
    below can be — so a real change falls through to the full
    regeneration path and gets a fresh LLM narrative, exactly as if
    reload_ai_summary=True had been passed.

    core_data_changed (reload_ai_summary=True) always skips SERVING from
    this cache — a forced reload must produce fresh prose. The lookup
    itself still runs either way (a cheap indexed Postgres read next to
    the LLM call this cache exists to avoid), purely so the response's
    `stale` flag still correctly reports a graph-side change even on a
    request that also happens to pass reload_ai_summary=True — `stale`
    reports the graph signal only (core_data_changed is never folded
    into it; see StalenessCheck.stale's docstring for why), so without
    this lookup a caller who set reload_ai_summary=True would silently
    lose the graph signal from the response instead of just no longer
    needing it to decide whether to serve the cache.

    Returns (response, staleness) on a genuine hit, or (None, staleness)
    otherwise — callers always get a staleness object either way, to
    carry into _generate_report_generate_fresh on a miss.
    """
    ...
    if cached_report is not None:
        ...
        # cached_report["generated_at"] is report_artifacts' own native
        # Postgres timestamp column for this exact draft (distinct from
        # cached_content["generated_at"], a string baked into the JSON
        # body) — the correct cache_generated_at reference point for
        # THIS report, never case_ai_summary_store.updated_at, which
        # tracks a different cache entirely.
        ...
        # StalenessCheck is frozen and takes plain booleans, so folding
        # in content_unchanged alongside AI-31's own timestamp signal is
        # a direct construction rather than a second call into
        # evaluate_cache_staleness — see this function's own docstring
        # for why both detectors matter.
        ...
        if not core_data_changed and content_unchanged:
            ...
            # override could still have moved (a plan modification or
            # revert doesn't touch related_network at all) — so the
            # Decision & Override Log is still re-derived fresh here and
            # spliced into the cached narrative rather than trusted from
            # cached_content["decision_log"].
            ...
            # technique apply_step_override_to_summary already uses to
            # overlay a live override onto /plan's cached prose.
            ...
            _log_call(req.case_id, username, "success", int(duration_seconds * 1000), "/generate_report")
            return (
                {
    ...
    # No report has ever been generated for this case_id — a genuine
    # first run, not staleness. graph_changed is reported False here for
    # the same reason every other cached-narrative route treats a
    # missing cache as "nothing to compare, no signal" rather than
    # guessing (see core.narrative_staleness.is_graph_newer's docstring).
    return None, StalenessCheck(core_data_changed=core_data_changed, graph_changed=False)


def _generate_report_generate_fresh(
    req: ReportGenerationRequest,
    username: str,
    token: str,
    case_data: Dict[str, Any],
    ...
    runner,
    related: Dict[str, Any],
    related_envelope: Dict[str, Any],
    plan_override,
    staleness: StalenessCheck,
    start: float,
) -> Dict[str, Any]:
    """Assemble the Decision & Override Log, run the LLM narrative call,
    splice in the deterministic sections, persist, and shape the
    response. Reached only when _generate_report_try_cache returned
    (None, staleness)."""
    # --- Decision & Override Log assembly (Report Design ACTIONS #3) ---
    # Deterministic, non-LLM formatting over two inputs already in hand:
    # the case's investigation-plan override (fetched fresh by the
    # caller, before the cache check) and the rejected entries out of
    # the Related Network (also fetched fresh by the caller). No graph
    # or DB call of its own — see reasoning_layer/decision_log.py.
    ...
    # rules_fired for the report narrative is a live Neo4j read, same as
    # /intake and /plan (see fetch_live_graph_findings) — case_data
    # (resolved via CS-4 warm lookup -> Postgres fallback -> ai_summary
    # body) NEVER carries rules_fired at all
    ...
    # investigator's reject/revert, or a rule that fired since the last
    # cached draft, is reflected in this report immediately.
    live_report_rules_fired = fetch_live_graph_findings(req.case_id, subject_id).get("rules_fired", [])

    # similar_cases for the report narrative is a live Neo4j read, same
    # pattern as rules_fired immediately above and the same helper
    # /similar_cases and /risk_assessment already use (see
    # fetch_live_similar_cases) — case_data NEVER carries similar_cases
    # at all (core.persistence_filters.strip_graph_derived_fields strips
    # the whole top-level section before every write, by design, so a
    # stale match list is never served), so case_data.get("similar_cases")
    # was always None here regardless of whether /similar_cases had ever
    # been run for this case, and the "Similar Cases" section always
    # fell through to the prompt's own "No similar cases identified."
    # fallback (config/prompts.py). Reading it fresh means the report
    # reflects the graph as it stands right now, exactly like every
    # other tab.
    live_similar_cases = fetch_live_similar_cases(req.case_id)

    # Inject the computed network and decision log into the case context
    # the prompt serialises, so the LLM narrates THESE facts (never
    # adds, removes, or reorders them — REPORT_GENERATION scope carries
    # no tools, so the LLM cannot re-query the graph or Postgres itself
    # either).
    #
    ...
    # agent_summary_cache / provenance_trail / network_match_flag, keeps
    # only the Primary Subject, strips rejection-audit attribution
    # (who/when) from rules_fired instances, trims risk_assessment down
    # to scores, similar_cases down to count + similarity signal,
    # investigation_plan down to steps, and collapses prior-guilty case
    # lists to a count. See reasoning_layer/report_llm_context.py for
    # the full rationale.
    ...
    # graph result, not anything the LLM produced — the LLM narrates, it
    # does not decide inclusion (mirrors AI-14's similar_cases pattern).
    sections: dict = {}
    ...
    # produced for it (AI-42). Until this fix, this splice only ran on
    # the report-cache-hit path — a freshly-generated report relied on
    # the LLM to render this section itself, which is exactly the
    # per-entry formatting REPORT_GENERATION_PROMPT's "No AI involved in
    # content, just formatting" rule (Report Design ACTIONS #3) rules
    # out. decision_log_result is already the same build_decision_log
    # output used for the cache-hit case, so this makes every report —
    # fresh or cached — get identical, deterministic text for this
    # section.
    ...
    # REPORT_GENERATION_PROMPT's placeholder under that header) — a
    # prior fix (AI-40) moved its per-entry formatting out of the prompt
    # entirely after the LLM was observed writing "not recorded" for
    # notation fields it was actually given real values for (see
    # reasoning_layer.decision_log.render_reviewed_and_excluded_markdown's
    # docstring). This splice is what actually populates the section's
    # body.
    assistant_text = replace_markdown_section(
        assistant_text,
        "Reviewed and Excluded Connections",
    ...
    # investigator-facing complaint number, never the internal case_id
    # — see core.case_store.get_complaint_number. case_id remains
    # report_content's own persisted key (below) and save_report's DB
    # key; only DISPLAY values change here.
    complaint_number = get_complaint_number(case_data) or req.case_id
    ...
    # authoritative one (the AppWorks-saved report is). A write failure
    # here must not fail this investigator-facing response; Neo4j + CS-4
    # already produced the authoritative content above.
...
def run_generate_report(req: ReportGenerationRequest, username: str, token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        #
        # Run BEFORE the report cache check (not just in the fresh-
        ...
        #
        # Kept inline here rather than factored into a shared helper with
        # /generate_report/pdf's identical block below — see module
        # docstring on why the two routes' pipelines stay independent.
        try:
        ...
        # cached report was generated. Kept inline for the same reason
        # as the Related Network block above.
        try:
        ...
        cache_response, staleness = _generate_report_try_cache(
            req, username, case_data, data_source, related, plan_override, start
        )
        if cache_response is not None:
            return cache_response

        return _generate_report_generate_fresh(
            req,
            username,
            token,
            case_data,
        ...
        _log_call(req.case_id, username, "auth_error", int((time.time() - start) * 1000), "/generate_report")
        raise HTTPException(status_code=401, detail=str(exc)) from exc
    except HTTPException:
        raise
    except Exception as exc:
        logger.exception("Report generation route failed for case_id=%s", req.case_id)
        _log_call(req.case_id, username, "error", int((time.time() - start) * 1000), "/generate_report")
        raise HTTPException(status_code=500, detail=f"Report generation failed: {exc}") from exc
    finally:
        logger.info("POST /generate_report completed for case_id=%s", req.case_id)


def _generate_report_pdf_try_cache(
    req: ReportGenerationRequest,
    username: str,
    case_data: Dict[str, Any],
    related: Dict[str, Any],
    ...
    freshly-derived Decision & Override Log into the cached narrative
    markdown (no LLM call), then render THAT markdown to PDF instead of
    returning it as JSON. Unlike /generate_report, this route has no
    JSON body to carry a `stale` flag in, so it doesn't need a
    StalenessCheck out of this — a plain Optional[Response] is enough.
    """
    ...
    cached_report_markdown = (cached_content.get("standard_sections") or {}).get("report_markdown", "")
    resolved_report_markdown = replace_markdown_section(
        cached_report_markdown,
        "Decision & Override Log",
        decision_log_result["decision_log_markdown"],
    )
    # Same treatment for Reviewed and Excluded Connections — see
    # /generate_report above for why.
    resolved_report_markdown = replace_markdown_section(
        resolved_report_markdown,
        "Reviewed and Excluded Connections",
        render_reviewed_and_excluded_markdown(cached_related_network),
...
def _generate_report_pdf_generate_fresh(
    req: ReportGenerationRequest,
    username: str,
    token: str,
    case_data: Dict[str, Any],
    ...
    """Assemble the Decision & Override Log, run the LLM narrative call,
    splice in the deterministic sections, persist, and render the PDF.
    Reached only when _generate_report_pdf_try_cache returned None."""
    # --- Decision & Override Log assembly (Report Design ACTIONS #3) ---
    ...
    # pattern /intake and /plan already use via fetch_live_graph_findings)
    # — case_data never carries rules_fired at all
    ...
    # Splice the deterministically-rendered Decision & Override Log
    # section into the LLM's markdown — identical fix to /generate_report
    # above (AI-42), applied here too since this route runs its own
    # independent LLM call rather than reusing that one.
    assistant_text = replace_markdown_section(
        assistant_text,
        "Decision & Override Log",
        decision_log_result["decision_log_markdown"],
    ...
    # /generate_report above, applied here too since this route runs its
    # own independent LLM call rather than reusing that one.
    assistant_text = replace_markdown_section(
        assistant_text,
        "Reviewed and Excluded Connections",
        render_reviewed_and_excluded_markdown(related.get("related_network", [])),
    ...
    # draft history for a given case_id. A write failure here must not
    # fail this investigator-facing PDF download. The PDF route has no
    # JSON body to surface persisted-state metadata in (unlike
    # /generate_report's "persisted_to_postgres"), so the return value
...
def run_generate_report_pdf(req: ReportGenerationRequest, username: str, token: str) -> Response:
    ...
    report_artifacts (D.5) exactly as /generate_report persists it — the
    two routes read and write the same drafts, so calling one after the
    other never triggers a redundant LLM call.
    """
    # Deferred — see module docstring for why.
    from api.server import _resolve_case_store
    ...
    try:
        ...
        # rationale on why this runs BEFORE the cache check. Kept inline
        # rather than shared — see module docstring.
        try:
            related_envelope = assemble_related_network(req.case_id, subject_id)
            related = related_envelope["result"]
        except (ValueError, GraphUnavailableError, Neo4jError) as exc:
        ...
        # investigation_plan_overrides (Section D.6) — fetched fresh
        # here, before the cache check, same as /generate_report.
        try:
            plan_override = get_override(req.case_id)
        except Exception as exc:
            logger.warning(
        ...
        cache_response = _generate_report_pdf_try_cache(req, username, case_data, related, plan_override, start)
        if cache_response is not None:
            return cache_response

        return _generate_report_pdf_generate_fresh(
            req, username, token, case_data, subject_id, runner, related, related_envelope, plan_override, start
        )
    except AppworksSessionExpiredError as exc:
        logger.warning("generate_report/pdf route: AppWorks session expired for case_id=%s", req.case_id)
        _log_call(req.case_id, username, "auth_error", int((time.time() - start) * 1000), "/generate_report/pdf")
        raise HTTPException(status_code=401, detail=str(exc)) from exc
    except HTTPException:
        raise
    except Exception as exc:
        logger.exception("Report PDF generation route failed for case_id=%s", req.case_id)
        _log_call(req.case_id, username, "error", int((time.time() - start) * 1000), "/generate_report/pdf")
