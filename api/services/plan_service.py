...
Internal structure (post-refactor, behavior unchanged): run_plan is a
thin orchestrator over _plan_cache_hit (Optional[Dict] — None means "no
usable cache/override, fall through") and _plan_fresh_run (always
returns a response dict), sharing _shape_plan_graph_findings and
_log_call so the response shape and audit-log call site can't drift
...
def _shape_plan_graph_findings(rule_aware_tasks: Any, rules_fired: Any) -> Dict[str, Any]:
    """Build the response's `graph_findings` block for /plan — the
    rule-aware task recommendations (AI-16) plus the fired-only rules
    they were derived from (Section 8.5 requires these displayed
    separately from the generic steps). `rules_fired` is the RAW block;
    fired_rules_only is applied here so both callers (cache-hit's live
    read, fresh-run's live read) can't drift on this trim.
    """
    return {
...
def _log_call(case_id: str, username: str, status: str, latency_ms: int) -> None:
    """Single call site for the /plan audit-log write, so every branch
...
def _plan_cache_hit(
    ...
    """Answer from the already-resolved case_data's cached agent_summary
    WITHOUT calling the LLM again, if there's a usable one.

    The investigation_plan_override (Section D.6) is checked FIRST,
    regardless of staleness: once an investigator has modified the plan,
    that modification is authoritative, so a cache hit is served even
    when a refresh was otherwise indicated — a fresh LLM run would only
    be discarded in favour of override["modified_steps"] anyway (see
    run_plan_pipeline in _plan_fresh_run), so skipping straight to it
    here saves the wasted LLM call. Only an explicit reload_ai_summary=True
    bypasses the cache when NO override exists; it still falls through to
    a fresh agent run in that case, same as before.

    Returns None (never raises) if there's no usable cache — callers
    should fall through to _plan_fresh_run in that case.
    """
    if staleness.should_auto_refresh and override is None:
        return None

    cached_summary = get_route_summary_text(case_data, "plan")
    cached_plan = case_data.get("investigation_plan")
    if cached_summary is None or not isinstance(cached_plan, dict):
        return None
    ...
    if override is not None:
        ...
        # override, but cached_summary is still the pre-override LLM
        # markdown pulled straight from case_ai_summary_store — without
        # this, the investigator reads AI-generated steps while
        # graph_findings/meta already say User Modified.
        cached_summary = apply_step_override_to_summary(cached_summary, override["modified_steps"])
    else:
    ...
    # rule_aware_tasks / rules_fired are ALWAYS a live Neo4j read, cache
    # hit or not — cached_plan never carries rule_aware_tasks and
    # case_data never carries rules_fired at all (see
    # core.persistence_filters.strip_graph_derived_fields), so a rejected
    # inference removes the task it justified immediately instead of
    # waiting for a full case reload.
    ...
    return {
        ...
        "details": {
            ...
            "graph_findings": _shape_plan_graph_findings(live_rule_aware_tasks, live_findings.get("rules_fired")),
            "meta": {
                ...
                # AI-32: independent of plan_stale above — plan_stale is
                # specifically about a saved HUMAN OVERRIDE lagging behind
                # case data (Section E.5); stale is about the underlying
                # AI-GENERATED narrative itself. Both can be true at once
...
def _plan_fresh_run(
    ...
    """Run the LLM plan agent, apply the plan pipeline (parsing + human-
    override application), persist, and shape the response. Reached only
    when _plan_cache_hit returned None."""
    ...
    # plan_section/investigation_plan here are the PURE, un-overridden LLM
    # output (see run_plan_pipeline) — this is intentional; the warm store
    # and Postgres must only ever hold the AI baseline so
    ...
    # POST /plan/revert_to_ai correct: the cached "AI" text must never be
    # the override's own text, or reverting has nothing genuine to fall
    # back to. The override is spliced into a SEPARATE response-only copy
    # below (response_agent_summary) — never into what gets cached here.
    # Carries forward any other route's already-cached entry for this
    # case_id.
    ...
    # Response-only override splice — mirrors the /plan CACHE-HIT branch
    # exactly: the persisted/cached text stays the pure AI baseline (just
    # written above); only what THIS response returns to the caller
    # reflects the override, the same way a cache hit never mutates
    # cached_summary/cached_plan before persisting anything.
    ...
    # rule_aware_tasks above — case_data (resolved before this request)
    # never carries rules_fired at all (see
    ...
    live_plan_rules_fired = fetch_live_graph_findings(req.case_id, plan_subject_id).get("rules_fired", [])
    ...
    return {
        ...
        "details": {
            ...
            "graph_findings": _shape_plan_graph_findings(
                investigation_plan.get("rule_aware_tasks"), live_plan_rules_fired
            ),
            "meta": {
                "data_source": data_source,
                "plan_source": plan_source,
...
def run_plan(req: PlanRequest, username: str, token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # /similar_cases' own entry check fires /intake if THAT is missing
        # — this route only needs to know about the one route immediately
        # before it.
        ...
        # merge_agent_summary_cache/persist_case_session calls overwrite
        # this exact "plan" cache entry — reading it late would make
        # every override look stale (Section E.5), and would make the
        # AI-32 graph check below compare against the run that is about
        # to happen instead of the one actually cached.
        #
        ...
        # compute_plan_staleness's manual-edit check inside _plan_cache_hit.
        # Reading the shared column here made /plan look "fresh" whenever
        # ANY other tab refreshed, even though nothing about /plan's own
        ...
        # generated_at they judged staleness against. Like /similar_cases
        # and /risk_assessment, /plan has no separate "pipeline" stage of
        # its own (rule_aware_tasks/rules_fired are already a live,
        # always-fresh Neo4j read regardless of cache state — see
        # fetch_live_rule_aware_tasks/prepare_plan_context). staleness.stale
        # reports the graph trigger only to the caller (see
        # StalenessCheck.stale's docstring for why core_data is excluded);
        # should_auto_refresh (the gate _plan_cache_hit checks) only fires
        # on an explicit reload_ai_summary=True — a graph-only change no
        # longer forces this route to regenerate its narrative on its own
        # (see StalenessCheck.should_auto_refresh's docstring).
        ...
        cache_hit = _plan_cache_hit(
            req, username, case_data, data_source, staleness, override, plan_generated_at_before_call, start
        )
        if cache_hit is not None:
            return cache_hit

        return _plan_fresh_run(
            req, username, token, case_data, data_source, staleness, override, plan_generated_at_before_call, start
        )
