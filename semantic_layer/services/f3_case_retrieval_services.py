...
# #
# # Strategy:
# #   Stage A (broad candidate pool):
# #     - Resolve AllegationType IDs from current case allegations
# #     - Fetch all Allegations_All rows per type ID
# #     - For each allegation row, resolve parent Workfolder
# #     - Fetch Workfolder_FinancialRelationship → sum Financial_Calculated
# #
# #   Stage B (manifest filters):
# #     - required_status  (WorkfolderStatus must match)
# #     - similarity_lookback_years  (WorkfolderDateReceived within N years)
# #     - max_results_per_type  (cap results per allegation type)
# #
...
#     Per API guide: GET /entities/Workfolder/items/{wf_id}/relationships/Workfolder_FinancialRelationship
#     Returns sum of all Financial_Calculated values found (more than one record is possible).
...
# ) -> list:
#     """
#     Apply manifest constraints.
#     All parameters come from manifest config — nothing hardcoded.
#     """
...
#             if case_status != required_status_norm:
#                 continue
...
#       - Query Allegations_All by each type ID (per API guide)
#       - For each matching workfolder, fetch Workfolder_FinancialRelationship
#         and sum Financial_Calculated (per API guide)
...
#         # Stage A: broad candidate fetch from Allegations_All per type
#         seen_pair: set = set()
#         wf_cache: dict = {}        # wf_id → Properties dict
#         fin_cache: dict = {}       # wf_id → financial_calculated (float or None)
...
#                 # Fetch workfolder properties (cached)
...
#                 # Fetch financial_calculated per API guide (cached per workfolder)
...
#                     "summary":              summary,
#                     "status":               wf_props.get("WorkfolderStatus") or None,
#                     "date_received":        wf_props.get("WorkfolderDateReceived") or None,
#                     "financial_calculated": fin_cache[wf_id],
#                 })
#     else:
#         # Legacy traversal path (kept for when enable_broad_fetch_stage=false)
...
#     # fallback_to_raw is a permission-gated flag. When false (default),
#     # filtered results are returned as-is even if empty.
...
#                 "allegation_id": alleg_id,
#                 "type_id":       type_id,
#                 "status":        props.get("Allegations_Status") or props.get("Allegations_AllegationStatus") or None,
