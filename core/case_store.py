...
from core.persistence_filters import strip_graph_derived_fields
...
def persist_case_session(case_id: str, ai_summary: Dict[str, Any]) -> None:
    ...
    FIRST — this is the one and only write path into
    case_ai_summary_store, so it is the
    single choke point that keeps rule results, risk-score graph
    add-ons, and network-match data out of Postgres. Only the AI-written
    narrative text and plain AppWorks-sourced data are ever persisted;
    the graph-derived fields are always re-read live from Neo4j instead
    (see api.pipeline_execution.fetch_live_graph_findings and friends).
    The in-memory CASE_STORE argument this function receives is left
    untouched by the stripping — only the persisted copy is affected.
    """
    ai_summary_to_persist = strip_graph_derived_fields(ai_summary)
